...
| `name` | string | Log prefix. Truncated to `concurrentlyOptions.prefixLength` (default 10). |
...
5. Spawn user commands concurrently (`sh -c` / `cmd /c`). `tunnelEnv` overlays `env` when tunneling is on. `handleInput: true` inherits stdin; `false` uses null stdin. `prefixColors: auto` cycles colors when `prefixColor` is unset.
...
5. **Process manager is native**, not the `concurrently` npm package. Wired: concurrent spawn, names, prefixes (including `prefixColors: auto`), `handleInput`, env/`tunnelEnv`, cwd, before/after, kill-others-on-failure, SIGINT process-group cleanup. Other concurrently keys deserialize and are ignored.
